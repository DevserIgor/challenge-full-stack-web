  ...
  <div>
    ...
    <div id="edit" style="display: none">
      <form class="pl-5 pt-15">
        ...
        <div class="d-flex mt-5">
          <label class="label-edit">RA</label
          ><input
            :disabled="edit ? true : false"
            class="input-edit"
            placeholder="Registro Acadêmico"
            v-model="tempStudent.register"
            type="text"
          />
        </div>
        <div class="d-flex mt-5">
          <label class="label-edit">CPF</label
          ><input
            v-mask="'###.###.###-##'"
            :disabled="edit ? true : false"
            class="input-edit"
            placeholder="CPF"
            v-model="tempStudent.cpf"
            type="text"
          />
        </div>
        <br /><br />
...
export default {
  ...
  methods: {
    searchstudents: function () {
      ...
      axios
        .get(`http://localhost:3000/users?search=${this.searchinput}`, headers)
        .then((response) => {
    ...
    newStudent: function () {
      ...
      this.edit = false;
      this.tempStudent = {};
      document.getElementById('list').style.display = this.new
        ? 'none'
        : 'block';
      document.getElementById('edit').style.display = this.new
        ? 'block'
        : 'none';
    },
    deleteStudent: function (item) {
      if (confirm('Quer mesmo excluir este cadastro?') == true) {
        axios
          .delete(`http://localhost:3000/users?id=${item.id}`, this.headers)
    ...
    editStudent: function (item) {
      this.edit = true;
      this.new = false;
      document.getElementById('list').style.display = this.edit
        ? 'none'
        : 'block';
      document.getElementById('edit').style.display = this.edit
        ? 'block'
        : 'none';
    ...
    creatStudent: function () {
      ...
      if (this.new) {
        axios
          .post('http://localhost:3000/users', this.tempStudent, headers)
          .then(function (response) {
            alert(response.data.status);
        ...
        axios
          .put(
            `http://localhost:3000/users?id=${this.tempStudent.id}`,
            this.tempStudent,
            headers
          )
          .then(function (response) {
